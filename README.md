...
Before starting the polling you will log into Arlo with your username/password. Then start polling the S3 presigned url since Arlo will initiate the out-of-band EMAIL MFA process.
...
### Arlo
* You will likely want to create an Arlo account dedicated to this purpose and the share your cameras/etc. with this account
...
* You are not already using SES incoming email since this solution will set the active receipt rule set to the one it created - which could impact any existing receipt rule sets you already have
...
* The Google side is free (Gmail and Voice)
...
## Deployment
* Simply run `./deploy.sh <stack> <forward-to-email> <s3-bucket-for-sam-artifacts>` and follow the guided prompts that SAM provides

Example

~~~bash
./deploy.sh arlo-mfa arlo@mfa.domain.com s3-bucket-name-for-sam-artifacts
~~~

## Post Deployment
* Manually make the receipt rule set the active one
    ...
    def Login(self, username, password, mfa_prestage_url=None):
        ...
        if body["mfa"] and self.mfa_prestage_url:
            ...
            response = standard_requests.post(
                ...
                json={'email': factor['displayName']}
            )
            mfa_url = response.text
